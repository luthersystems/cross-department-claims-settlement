       ...
       [invoke-handler-helper (resp-body)
         ...
         (let* ([req-id (get resp-body "request_id")]
                ...
                [handler-fn (get reg-handlers handler-name)])
                 (cc:infof (sorted-map "handler-fn" handler-fn "ctx" ctx) "handler fn and")
           (when msp 
...
(defun mk-connector-events ()
  (cc:infof (sorted-map) "mk-connector-events invoked")
  (let ([state (sorted-map)])
    (labels
      ([num-events () (default (get state "ctr") 0)]
       ...
       [raise
        (cc:infof (sorted-map) "mk-connector-events/raise invoked")
...
(defun do-transition (obj-factory transition)
  (cc:infof (sorted-map "transition" transition) "do-transition invoked")
  (let* ([obj-handler-name (or (obj-factory 'name)
         ...
         (cc:infof (sorted-map "events in do-transition" events) "events in do-transition")
    (when put-obj
      (obj-factory 'put put-obj)) ; this is where state is changed
    (when del-obj
...
(defun trigger-connector-object (obj-factory obj-id resp &optional ctx)
  (cc:infof (sorted-map "resppp" resp) "trigger-connector-object invoked")
  (let* ([obj-id (or obj-id
         ...
         [transition (obj 'handle resp ctx)])
           (cc:infof (sorted-map "transition" transition) "trigger-connector-object transition")
